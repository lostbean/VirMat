...
runPacker ::
    (Packer v, Buildable S2 v, Norm Double v) =>
    Int -> Box v -> SetPoint v -> IntMap (S2 v) -> (SetPoint v, IntMap (S2 v))
runPacker n box ps wall =
    let
        interDeUni = 2
        smooth = 15
...
getDisplacement sp i ps time =
    let
     ...
     in
        ...
        if l > 0.5 * r then disp &* (0.5 * r / l) else disp

updateSP ::
    (Packer v, Norm Double v) =>
    Box v ->
    IntMap (S2 v) ->
    SetPoint v ->
    SetPoint v ->
    ...
    Double ->
    SetPoint v
updateSP box tri sp0 sp1 damp time =
    let
        conn = findPointConn tri
        update i =
            let
                ...
                new ps =
                    let
                     ...
                     in
                        keepInBox box newP
             in
                case IM.lookup i conn of
                    Just ps -> (sp1 ! i){point = new ps}
                    _ -> sp1 ! i
     in
        V.generate (V.length sp1) update
                            ...
                            add c a $
                                add b a $
                                    add c b $
                                        add a c acc
         in
            IM.foldl' func IM.empty

    keepInBox Box2D{..} (Vec2 x y) =
        let
