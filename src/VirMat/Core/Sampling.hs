...
import System.Random.Stateful (AtomicGenM)
...
instance Distribution LogNormal where
    ...
    distFunc (LogNormal{..}) x =
        let
         ...
         in
            if x > 0 then logNormalScale * (a / b) else 0
    distInterval (LogNormal{..}) =
        let
            mu = log logNormalMode + sigma2
            sigma2 = (2 / 3) * log (logNormalMean / logNormalMode)
            sigma = sqrt sigma2
...
instance Distribution Normal where
    ...
    distInterval (Normal{..}) =
        let
         ...
         in
            (normalMean - threeS, normalMean + threeS)
...
instance Distribution Uniform where
    ...
    distInterval (Uniform{..}) =
        let
            a = sqrt (3 * uniformVar)
         in
            (uniformMean - a, uniformMean + a)
    ...
    distFunc (CustomDist{..}) x =
        let
            ...
            func i
                ...
                | otherwise = i - 1
              where
                middle = 0.5 * ((customBinCenter V.! i) + (customBinSize V.! (i - 1)))
         in
            maybe 0 ((customDist V.!) . func) r
    distInterval (CustomDist{..}) =
        let
...
divConAreaBin func ia@IB{..} =
    let
     ...
     in
        if abs (area1 + area2 - area) > integralerror
...
integrateMDist mdist =
    let
     ...
     in
        ...
        V.scanl' (\acc x -> DiscValue (xmax x) (discY acc + area x)) initDisc areas
    ...
    let
        ...
        finder ia ib
          ...
          where
            ...
            by = discY b
     in
        if size > 0 then finder 0 (size - 1) else Nothing
...
-}
sampleVol :: (Ord b, Num b) => MultiDist -> AtomicGenM PureMT -> (Double -> b) -> b -> IO (Vector Double)
