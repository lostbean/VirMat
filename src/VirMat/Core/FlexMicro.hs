...
instance FlexMicroBuilder Vec2 where
    ...
    modifyGrainProps func fm@FlexMicro2D{..} =
      ...
      where
        foo gid gp = setPropValue gp (func gid gp)
...
instance FlexMicroBuilder Vec3 where
    data FlexMicro Vec3 a
        = FlexMicro3D
        { flexGraph3D :: MicroGraph a MeshConn (Vector Int) Int
        , -- \^ Microstructure graph
          flexPoints3D :: Vector Vec3D
        }
        -- \^ Controls points (vertices, edges and faces)
        deriving (Show)
    mkFlexMicro vm = getFlexFaces3D vm $ getFlexEdges3D vm $ getFlexGrainsAndVertices3D vm
    renderFlexMicro renders n fm@FlexMicro3D{..} =
        let
...
getFlexGrainsAndEdges2D MicroGraph{..} =
    let
...
getFlexFaces2D vm@MicroGraph{..} (FlexMicro2D fm ps) =
    let
        psSize = V.length ps
...
getFlexGrainsAndVertices3D MicroGraph{..} =
    let
...
getFlexEdges3D vm@MicroGraph{..} (FlexMicro3D fm ps) =
    let
        psSize = V.length ps
...
getFlexFaces3D MicroGraph{..} (FlexMicro3D fm ps) =
    let
        psSize = V.length ps
        fs_clean = fst $ HM.foldlWithKey' foo (V.empty, psSize) microFaces
...
addGrainAttrs ::
    (RenderElemVTK a, FlexMicroBuilder v) =>
    GrainID ->
    FlexMicro v g ->
    [RenderGrainProp g] ->
    VTK a ->
    VTK a
addGrainAttrs gid fm renders vtk =
    let
        foo (RenderGrainProp (name, func)) =
            let
                add x v = addCellAttr v (mkCellAttr name (\_ _ _ -> x))
             in
                maybe id add (func gid (flexGrainProp gid fm >>= getPropValue))
     in
        L.foldl' (flip foo) vtk renders
...
renderGrains3D gid renders n fm@FlexMicro3D{..} =
    let
     ...
     in
        maybe V.empty (HS.foldl' func V.empty) grainConn

renderFace3D :: FaceID -> Int -> FlexMicro Vec3 a -> Maybe (VTK Vec3D)
renderFace3D fid n FlexMicro3D{..} =
    renderSubTwo n flexPoints3D
