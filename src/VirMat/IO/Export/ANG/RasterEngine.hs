...
fillBottomFlatTriangle g p1 p2 p3 =
    let
...
fillTopFlatTriangle g p1 p2 p3 =
    let
...
boundingBox g (Vec2 ax ay) (Vec2 bx by) (Vec2 cx cy) =
    let
...
mkBackground g =
    let
     ...
     in
        V.generate (ny * nx) (nullPoint . toXY)
...
flexmicroToANG n step box fm@FlexMicro2D{..} =
    let
        a0 = angInit step box
            ...
            let
                ...
                fillTriangle (p1, p2, p3) =
                    let
                     ...
                     in
                        mapM_ (\xy -> let pos = toPos xy in VM.write m pos (mkPoint q 1 xy 1)) xys
             in
