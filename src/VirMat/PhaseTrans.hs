...
applyParentProp simProduct nullprop props =
    let
        table = IM.fromList props
        func gid _ = IM.findWithDefault nullprop (unGrainID gid) table
     in
        modifyGrainProps func $ mkFlexMicro (grainSet simProduct)
...
getParentProp n microParent simProduct =
    let
