...
calc2DGrainLength ::
    (AbelianGroup v, DotProd Double m, Norm Double m, SubZero (SubOne v), v ~ m Double) =>
...
calc3DGrainVolume d vs vmesh n = V.foldl' func 0 vmesh
  where
    func acc m = acc + calcSubTwoVolume d vs m n
...
calcCentroidSubOne ::
    (SubZero (SubOne v), LinearMap Double m, v ~ m Double) =>
    ...
    func sb =
        let
         ...
         in
            ...
            fst $ V.foldl' foo (0, V.head ps) (V.tail ps)
  ...
  where
    ...
    func sb =
        let
         ...
         in
            -- ps > 2 guaranteed by 'mkSubOne'
            fst $ V.foldl' f (0, V.head ps) (V.tail ps)

withSubTwoTriangles :: (a -> Vec3D -> Vec3D -> Vec3D -> a) -> a -> Vector Vec3D -> MeshConn -> Int -> a
withSubTwoTriangles func x0 vs mesh n =
    let
        sb = mkSubTwoFromMesh vs mesh
        sbN = subdivideN n sb
        ps = subTwoPoints sbN
     ...
     in
        V.foldl' (\acc (a, b, c) -> func acc (ps V.! a) (ps V.! b) (ps V.! c)) x0 ts
...
getCentroid v = V.foldl' (&+) zero v &* k
  where
    k = recip . fromIntegral . V.length $ v
