...
class GenRandom v where
    ...
    genPoint :: AtomicGenM PureMT -> RVar Double -> IO (v Double)
...
instance GenRandom Vec2 where
    ...
    calcBox (Area area) (xRatio, yRatio) =
        let
            refRatio = xRatio
            k1 = yRatio / refRatio
            a = sqrt (area / k1)
    ...
    calcBoxFromDist rs ratio =
        let
            totalArea = V.foldl' (\acc d -> acc + diaToExt d) 0 rs
         in
            calcBox (1.1 * totalArea) ratio
...
instance GenRandom Vec3 where
    ...
    calcBox (Vol vol) (xRatio, yRatio, zRatio) =
        let
            refRatio = xRatio
            k1 = yRatio / refRatio
            k2 = zRatio / refRatio
...
getRandomGen :: Maybe Int -> IO (AtomicGenM PureMT)
getRandomGen a = case a of
    Nothing -> AtomicGenM <$> (newPureMT >>= newIORef)
    Just s -> AtomicGenM <$> newIORef (pureMT (fromIntegral s))
...
-}
genGrainDistributionByNumber ::
    (GenRandom v, Pointwise (v Double)) =>
